...
const wordsShuffled = shuffleArray(GAME_WORDS);

export const ScrambleWords = () => {
  const [words, setWords] = useState(wordsShuffled);
  ...
  const handleGuessSubmit = (e: React.FormEvent) => {
    ...
    if (errorCounter + 1 < maxAllowErrors) return;
    setIsGameOver(true);
  };
  ...
  const handleSkip = () => {
    ...
    setSkipCounter((oldVal) => oldVal + 1);
    setCurrentWord((oldWord) => {
      const foundWordIdx = words.findIndex((word) => word === oldWord);
      const newWord = words[foundWordIdx + 1];
      setScrambledWord(scrambleWord(newWord));
      return newWord;
    });
    setGuess('');
  ...
  const handlePlayAgain = () => {
    const wordsShuffled = shuffleArray(GAME_WORDS);
    setWords(wordsShuffled);
    setScrambledWord(wordsShuffled[0]);
    setGuess('');
    setPoints(0);
    setErrorCounter(0);
    setSkipCounter(0);
